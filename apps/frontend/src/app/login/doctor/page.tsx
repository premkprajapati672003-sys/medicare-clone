...
export default function DoctorLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      if (user.role !== 'DOCTOR') {
        setError('Access denied. This portal is strictly for medical staff.');
        return;
      }
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:3002/api/auth/google', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: credentialResponse.credential, role: 'DOCTOR' })
      });
      if (res.ok) {
        const user = await res.json();
        localStorage.setItem('hotdoc_user', JSON.stringify(user));
    ...
    >
      ...
      <div className="w-full max-w-[480px] bg-[#0c1222]/40 backdrop-blur-xl border border-white/20 rounded-2xl relative flex items-center justify-center overflow-hidden z-10 shadow-2xl mx-4">
        ...
        <div className="absolute top-6 right-6 z-20">
          <Link href="/login/patient" className="text-[#6688aa] hover:text-white text-sm font-medium transition-colors">
            Patient Login →
          </Link>
        </div>
        ...
        <div className="relative z-10 w-full px-8 py-12 pt-16">
          ...
          <p className="text-[#556677] text-sm text-center">
            Not a doctor?{' '}
            <Link href="/login/patient" className="text-blue-400 font-semibold hover:underline">Patient Login</Link>
          </p>
